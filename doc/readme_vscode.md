...
## Настраиваем VSCode IntelliSense

Как глобально настроить IntelliSense в VSCode, я не нашел. Поэтому настройки делаем per project.
Если файл `.vscode/c_cpp_properties.json` не существует, то при попытке его открытия сгенерируется
вполнегодный `c_cpp_properties.json`, который надо лишь немножко подтюнить.


Через VS Code Command Palette (Ctrl+Shift+P) набираем `C/C++: Edit Configurations (JSON)`.

Находим параметр `configurationProvider` (или создаём), устанавливаем:
```
"configurationProvider": "ms-vscode.cmake-tools"
```

Если IntelliSense не заработал, пробуем следующий способ.

Находим параметр `compileCommands` (или создаём), устанавливаем:
```
"compileCommands": "${workspaceFolder}/.build/${buildKit}/${buildKitTargetOs}-${buildKitTargetArch}/${buildType}/compile_commands.json",
```

Путь к файлу `compile_commands.json` должен совпадать с путем в опции `cmake.buildDirectory` настроек VSCode (`Settings.json`):
```
...
См. https://stackoverflow.com/questions/36122358/visual-studio-code-c-include-path, 
...
В опции `cmake.buildDirectory` можно задать путь для генерации сборочных скриптов и помещения туда выходных файлов. Это значение 
...
Ведущие собаководы тут https://stackoverflow.com/questions/23123431/arm-linking-error-uses-vfp-register-arguments-main-elf-does-not-on-windows-8
рекомендуть прописать путь к бинарникам тулсета в PATH.

Но это не так. Помогла замена линкера `LD` на `G++`:
```
...
## О пользе настройки выходного пути

Если настроить переменную `cmake.buildDirectory` в конфиге VSCode, и задать значение вида
`${workspaceFolder}/.build/${buildKit}/${buildKitTargetOs}-${buildKitTargetArch}/${buildType}`
то результаты компиляции не будут размазаны ровным слоем по каталогам с исходниками, а будут собраны
в одном месте, сгруппированы по китам, целевым платформам и версиям сборки:
```
.build
  GCC 7.3.0 i686-w64-mingw32
    win32-x86
      Debug
      Release
