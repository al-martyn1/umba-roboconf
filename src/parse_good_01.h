...
struct ExpressionItem
{
    ...
    struct expression_list_t
    {
        ...
        expression_list_t* getParentList()
        {
            if (!pParentItem)
                return 0;
...
#define ROBOCONF_PARSE_READ_LIST_BY_TEMPLATE( pTpl, it, E, readTo, expected, found ) \
                       readListByTemplate( pTpl, it, E, readTo, expected, found, __FILE__, __LINE__ )


//-----------------------------------------------------------------------------
inline
bool readListByTemplate( const char *pTpl
                       , expression_list_t::const_iterator &it
                       , const expression_list_t::const_iterator E
    ...
    for(; b!=e; ++b)
    {
        ...
        for( auto pLst : candyList )
        {
            ...
            for( ; it!=lst.end(); ++it)
            {
                ...
                if (b.isNameMatched(subLst[0].itemText))
                 ...
                 )
{
    ...
    {
        std::set<std::string> alters;
        splitToSet( path, alters, "|" );

        bool allowAny = false;
        if (alters.find("*")!=alters.end())
            allowAny = true;
        ...
        for( auto pLst : srcLists )
        {
            ...
            for( ; it!=lst.end(); ++it)
            {
                ...
                std::string txt = subLst[0].itemText;
                if (caseIgnore)
                    txt = toLower(txt);

                if (allowAny || alters.find(txt)!=alters.end())
                {
                    resLists.push_back(&subLst);
                }
            }
                 ...
                 )
{
    ...
    std::vector<std::string> pathParts;
    splitToVector( query, pathParts, '/' );
    if (caseIgnore)
        pathParts = toLower(pathParts);
    ...
    return executeQuery( srcLists, pathParts, pRes, caseIgnore );
}
