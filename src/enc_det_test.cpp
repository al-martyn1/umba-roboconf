...
int main(int argc, char* argv[])
{
    if (argc<2)
    {
        cout<<"Usage: enc_det_test input_file\n";
    ...
    size_t bomSize = 0;
    std::string encNameDetected = pEncApi->detect( text.data(), text.size(), bomSize );
    if ( encNameDetected.empty() )
    {
        cout<<"Invalid encoding\n";
    }
    else
    {
        cout<<"Encoding: "<<encNameDetected<<"\n";
    }

    cout<<"BOM size: "<<bomSize<<"\n";
    if (bomSize)
        text.erase(0, bomSize);

    UINT cp = pEncApi->getCodePageByName( encNameDetected );
    if (!cp)
        cout<<"Unknown codepage\n";
    else
    {
        cout<<"Codepage: "<<cp<<" "<<pEncApi->getCodePageInfo(cp)<<"\n"; //<<"";
        text = pEncApi->convert(text, cp, EncodingsApi::cpid_UTF8 );
        cout<<"Encoded text:\n"<<text<<"\n";
    }
...
/*
    //CompactEncDet::
    Encoding enc = 
    CompactEncDet::DetectEncoding(text.data(), text.size(), nullptr,  // URL hint nul
                                  nullptr,                      // HTTP hint
                                  nullptr,                      // Meta hint
                                  UNKNOWN_ENCODING,
                                  UNKNOWN_LANGUAGE,
    ...
    {
        cout<<"Encoding: "<<EncodingName(enc)<<"\n";
        cout<<"MIME Enc: "<<MimeEncodingName(enc)<<"\n";
    }
    ...
    CPINFOEXA cpinfo;
    for( UINT cp = 0; cp<65536; ++cp)
    {
        if (GetCPInfoExA( cp, 0, &cpinfo ))
        {
            cout<<cpinfo.CodePageName<<": "<<cp<<"\n";
        }
    }
*/
...
https://docs.microsoft.com/ru-ru/windows/desktop/Intl/code-page-bitfields
